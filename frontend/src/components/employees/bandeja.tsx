...
function Bandeja() {
    ...
    return (
        <div className="bandeja-container">
            ...
            {cargando && <p className="cargando">Cargando comentarios...</p>}
            {error && <p className="error">Error: {error}</p>}
            
            {!cargando && !error && (
                <div className="lista-comentarios">
                    {comentarios.length === 0 ? (
                        <p className="sin-comentarios">No hay comentarios disponibles</p>
                    ) : (
                        comentarios.map((comentario) => (
                            ...
                            >
                                ...
                                <p className="comentario-preview">{comentario.textCommentary.substring(0, 100)}...</p>
                                <span className="comentario-producto-id">Producto ID: {comentario.idProduct}</span>
                            </div>
                        ))
                    )}
                </div>
